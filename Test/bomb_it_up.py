...
class TestAll(unittest.TestCase):
    ...
    def test_search_by_error_name(self):
        ...
        for log in _logs:
            self.assertEqual(log['error_name'], random_error_name.lower(
            ), 'Search by error name [no filter] - search parameter and request field did not match!')
        # ----------------------------------------------------------------------
        ...
        for log in _logs:
            ...
            self.assertGreaterEqual(_logged_at_index_zero, _logged_at,
                                    'Search by error name, first datelimit and descending order - descending order not working')
        ...
        for log in _logs:
            ...
            self.assertLessEqual(_logged_at_index_zero, _logged_at,
                                 'Search by error name and first date limit - ascending order not working')
            _logged_at_index_zero = _logged_at
        # ----------------------------------------------------------------------
        ...
        for log in _logs:
            _logged_at = datetime.fromtimestamp(
                int(log['logged_at_unix']) / 1000)
            self.assertGreaterEqual(_logged_at_index_zero, _logged_at,
                                    'Search by error name, first date, last date limit - descending order not working')
            _logged_at_index_zero = _logged_at
        # ----------------------------------------------------------------------
        ...
        for log in _logs:
            _logged_at = datetime.fromtimestamp(
                int(log['logged_at_unix']) / 1000)
            self.assertGreaterEqual(_logged_at_index_zero, _logged_at,
                                    'Search by error name, first date, last date limit - ascending order not working')
            _logged_at_index_zero = _logged_at
        # ----------------------------------------------------------------------
        ...
        self.assertEqual(
            total, 2, 'Search by name, first date limit, last date limit, descending order and limit: Limit did not work.')
    ...
    def test_search_by_error_origin(self):
        ...
        self.assertEqual(len(logs), total,
                         'log length and total value not matching matching')

        for log in logs:
            log_origin = log['point_of_origin']
            if log_origin != self.test_error_insertion.__name__:
                self.assertEqual(log_origin, origin,
                                 'origin did not match! WOT WOT @.o')
        # ---------------------------------------------------------
    ...
    def test_search_in_between_dates(self):
        ...
        self.assertNotEqual(
            total, -1, "key 'total' is not present inside 'errors' dict")
        self.assertNotEqual(total, 0, "database is not populated yet!")
        self.assertEqual(total, len(
            logs), "Length of all logs and total did not match!")
        _logged_at_index_zero = logs[0]
        _logged_at_index_zero = _logged_at_index_zero[
            'logged_at_unix'] if 'logged_at_unix' in _logged_at_index_zero else None
        _logged_at_index_zero = datetime.fromtimestamp(
            int(_logged_at_index_zero) / 1000)
        for log in logs:
            _logged_at = datetime.fromtimestamp(
                int(log['logged_at_unix']) / 1000)
            self.assertGreaterEqual(_logged_at_index_zero, _logged_at,
        ...
        self.assertNotEqual(
            total, -1, "key 'total' is not present inside 'errors' dict")
        self.assertNotEqual(
            total, 0, "list is not supposed to be empty here now")
        self.assertEqual(total, len(
            logs), "Length of all logs and total did not match!")
        ...
        self.assertNotEqual(
            total, 0, "list is not supposed to be empty here now")
        self.assertEqual(
            total, limit, "List is supposed to trancate down to limit")
        self.assertEqual(total, len(
            logs), "Length of all logs and total did not match!")
        ...
        for _ in range(0, 50):
            ...
            if do_or_not_do == 0:
                ...
                if isinstance(result, int):
                    ...
                    status = False
                self.assertTrue(status, 'Mark Resolve: came False for {}'.format(
                    "No error - {}".format(_)))
    ...
    def test_all_error(self):
        ...
        self.assertGreaterEqual(
            total, 150, 'Test all error - Total is less than or equal to 150 now')
        logs = all_errors['log'] if 'log' in all_errors else list()
        self.assertEqual(
            len(logs), total, 'Test all error - log entry is not equal to total value')
        first_logged_at = logs[0]['logged_at_unix']
        for log in logs:
            logged_at = log['logged_at_unix']
            error_name = log['error_name']
            self.assertGreaterEqual(
                first_logged_at, logged_at, 'Test all error - descending is not working right for {}'.format(error_name))
    ...
    def test_all_debug(self):
        ...
        self.assertGreaterEqual(
            total, 100, 'There should be 100 debug messages on database at least')
        self.assertNotEqual(
            total, -1, "'total' key is not present in 'debugs' dict")
        self.assertNotEqual(total, 0, 'db is not populated! WT!')
        self.assertEqual(total, len(
            logs), "total and log length did not match!")
    ...
    def test_debug_by_user(self):
        ...
        self.assertEqual(total, len(
            logs), 'logs and length did not match with each other')
        self.assertNotEqual(
        ...
        first_log = datetime.fromtimestamp(
            int(logs[0]['logged_at_unix']) / 1000)
        last_log = datetime.fromtimestamp(
            int(logs[len(logs) - 1]['logged_at_unix']) / 1000)
        self.assertGreaterEqual(
            first_log, first_limit, "log time should be less than or equal to datetime")
        self.assertGreater(last_limit, last_log,
    ...
    def test_debug_by_origin(self):
        ...
        for log in logs:
            self.assertEqual(log['point_of_origin'], origin.lower(
            ), 'Origins are not matching up homie!')
